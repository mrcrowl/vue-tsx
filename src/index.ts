...
function processKey(hAttr: VNodeData, tsxAttr: { [key: string]: any }, key: string)
{
    if (/^on[A-Z]/.test(key))
    {
        ...
        hAttr.on = hAttr.on || {}
        hAttr.on[eventName] = tsxAttr[key]
    }
    else if (/^nativeOn[A-Z]/.test(key))
    {
        const eventName = key.slice(8).toLowerCase()
        hAttr.nativeOn = hAttr.nativeOn || {}
        hAttr.nativeOn[eventName] = tsxAttr[key]
    }
    else if (/^domProps[A-Z]/.test(key))
    {
        const property = key.slice(8, 9).toLowerCase() + key.slice(9)
        hAttr.domProps = hAttr.domProps || {}
        hAttr.domProps[property] = tsxAttr[key]
    }
    else switch (key)
    {
        case "key":
...
export function TSXRender(): PropertyDecorator
{
    return function (target: any, key: string)
    {
        ...
        createDecorator((componentOptions, k) =>
        {
            ...
            {
                const self = this;
                const f = function (tagName: string | ObjectConstructor, tsxAttr: {})
                {
                    let nodeData: VNodeData = { attrs: {} }
                    for (const key in tsxAttr)
                    {
                        nodeData = processKey(nodeData, tsxAttr, key)
                    }
...
export declare namespace VueTSX
{
    interface VueEvent<T> extends Event { target: EventTarget & T }
    interface VueUIEvent<T> extends UIEvent { target: EventTarget & T }
    interface VueDragEvent<T> extends DragEvent { target: EventTarget & T }
    interface VueFormEvent<T> extends Event { target: EventTarget & T }
    interface VueMouseEvent<T> extends MouseEvent { target: EventTarget & T }
    interface VueWheelEvent<T> extends WheelEvent { target: EventTarget & T }
    interface VueTouchEvent<T> extends TouchEvent { target: EventTarget & T }
    interface VueFocusEvent<T> extends FocusEvent { target: EventTarget & T }
    interface VueKeyboardEvent<T> extends KeyboardEvent { target: EventTarget & T }
    interface VueClipboardEvent<T> extends ClipboardEvent { target: EventTarget & T }
    interface VueAnimationEvent<T> extends AnimationEvent { target: EventTarget & T }
    interface VueTransitionEvent<T> extends TransitionEvent { target: EventTarget & T }
    interface VueCompositionEvent<T> extends CompositionEvent { target: EventTarget & T }

    interface DOMEvents<T>
    {
        // Clipboard Events
        onCopy?: (e: VueClipboardEvent<T>) => any
        onCut?: (e: VueClipboardEvent<T>) => any
        onPaste?: (e: VueClipboardEvent<T>) => any

        // Composition Events
        onCompositionEnd?: (e: VueCompositionEvent<T>) => any
        onCompositionStart?: (e: VueCompositionEvent<T>) => any
        onCompositionUpdate?: (e: VueCompositionEvent<T>) => any

        // Focus Events
        onFocus?: (e: VueFocusEvent<T>) => any
        onBlur?: (e: VueFocusEvent<T>) => any

        // Form Events
        onChange?: (e: VueFormEvent<T>) => any
        onInput?: (e: VueFormEvent<T>) => any
        onReset?: (e: VueFormEvent<T>) => any
        onSubmit?: (e: VueFormEvent<T>) => any
        onInvalid?: (e: VueFormEvent<T>) => any

        // Image Events
        onLoad?: (e: VueEvent<T>) => any
        onError?: (e: VueEvent<T>) => any // also a Media Event

        // Keyboard Events
        onKeyDown?: (e: VueKeyboardEvent<T>) => any
        onKeyPress?: (e: VueKeyboardEvent<T>) => any
        onKeyUp?: (e: VueKeyboardEvent<T>) => any
        ...
        onDragExit?: (e: VueDragEvent<T>) => any
        onDragLeave?: (e: VueDragEvent<T>) => any
        onDragOver?: (e: VueDragEvent<T>) => any
        onDragStart?: (e: VueDragEvent<T>) => any
        onDrop?: (e: VueDragEvent<T>) => any
        onMouseDown?: (e: VueMouseEvent<T>) => any
        onMouseEnter?: (e: VueMouseEvent<T>) => any
        onMouseLeave?: (e: VueMouseEvent<T>) => any
        onMouseMove?: (e: VueMouseEvent<T>) => any
        onMouseOut?: (e: VueMouseEvent<T>) => any
        onMouseOver?: (e: VueMouseEvent<T>) => any
        onMouseUp?: (e: VueMouseEvent<T>) => any

        // Selection Events
        onSelect?: (e: VueEvent<T>) => any

        // Touch Events
        onTouchCancel?: (e: VueTouchEvent<T>) => any
        onTouchEnd?: (e: VueTouchEvent<T>) => any
        onTouchMove?: (e: VueTouchEvent<T>) => any
        onTouchStart?: (e: VueTouchEvent<T>) => any

        // UI Events
        onScroll?: (e: VueUIEvent<T>) => any

        // Wheel Events
        onWheel?: (e: VueWheelEvent<T>) => any

        // Animation Events
        onAnimationStart?: (e: VueAnimationEvent<T>) => any
        onAnimationEnd?: (e: VueAnimationEvent<T>) => any
        onAnimationIteration?: (e: VueAnimationEvent<T>) => any

        // Transition Events
        onTransitionEnd?: (e: VueTransitionEvent<T>) => any
    }
...
declare global
{
    namespace JSX
    {
        ...
        interface IntrinsicElements
        {
            ...
            area: VueTSX.HTMLProps<HTMLAreaElement>
            article: VueTSX.HTMLProps<HTMLElement>
            aside: VueTSX.HTMLProps<HTMLElement>
            audio: VueTSX.HTMLProps<HTMLAudioElement>
            b: VueTSX.HTMLProps<HTMLElement>
            base: VueTSX.HTMLProps<HTMLBaseElement>
            bdi: VueTSX.HTMLProps<HTMLElement>
            bdo: VueTSX.HTMLProps<HTMLElement>
            big: VueTSX.HTMLProps<HTMLElement>
            blockquote: VueTSX.HTMLProps<HTMLElement>
            body: VueTSX.HTMLProps<HTMLBodyElement>
            br: VueTSX.HTMLProps<HTMLBRElement>
            button: VueTSX.HTMLProps<HTMLButtonElement>
            canvas: VueTSX.HTMLProps<HTMLCanvasElement>
            caption: VueTSX.HTMLProps<HTMLElement>
            cite: VueTSX.HTMLProps<HTMLElement>
            code: VueTSX.HTMLProps<HTMLElement>
            col: VueTSX.HTMLProps<HTMLTableColElement>
            colgroup: VueTSX.HTMLProps<HTMLTableColElement>
            data: VueTSX.HTMLProps<HTMLElement>
            datalist: VueTSX.HTMLProps<HTMLDataListElement>
            dd: VueTSX.HTMLProps<HTMLElement>
            del: VueTSX.HTMLProps<HTMLElement>
            details: VueTSX.HTMLProps<HTMLElement>
            dfn: VueTSX.HTMLProps<HTMLElement>
            dialog: VueTSX.HTMLProps<HTMLElement>
            div: VueTSX.HTMLProps<HTMLDivElement>
            dl: VueTSX.HTMLProps<HTMLDListElement>
            dt: VueTSX.HTMLProps<HTMLElement>
            em: VueTSX.HTMLProps<HTMLElement>
            embed: VueTSX.HTMLProps<HTMLEmbedElement>
            fieldset: VueTSX.HTMLProps<HTMLFieldSetElement>
            figcaption: VueTSX.HTMLProps<HTMLElement>
            figure: VueTSX.HTMLProps<HTMLElement>
            footer: VueTSX.HTMLProps<HTMLElement>
            form: VueTSX.HTMLProps<HTMLFormElement>
            h1: VueTSX.HTMLProps<HTMLHeadingElement>
            h2: VueTSX.HTMLProps<HTMLHeadingElement>
            h3: VueTSX.HTMLProps<HTMLHeadingElement>
            h4: VueTSX.HTMLProps<HTMLHeadingElement>
            h5: VueTSX.HTMLProps<HTMLHeadingElement>
            h6: VueTSX.HTMLProps<HTMLHeadingElement>
            head: VueTSX.HTMLProps<HTMLHeadElement>
            header: VueTSX.HTMLProps<HTMLElement>
            hgroup: VueTSX.HTMLProps<HTMLElement>
            hr: VueTSX.HTMLProps<HTMLHRElement>
            html: VueTSX.HTMLProps<HTMLHtmlElement>
            i: VueTSX.HTMLProps<HTMLElement>
            iframe: VueTSX.HTMLProps<HTMLIFrameElement>
            img: VueTSX.HTMLProps<HTMLImageElement>
            input: VueTSX.ChangeTargetHTMLProps<HTMLInputElement>
            ins: VueTSX.HTMLProps<HTMLModElement>
            kbd: VueTSX.HTMLProps<HTMLElement>
            keygen: VueTSX.HTMLProps<HTMLElement>
            label: VueTSX.HTMLProps<HTMLLabelElement>
            legend: VueTSX.HTMLProps<HTMLLegendElement>
            li: VueTSX.HTMLProps<HTMLLIElement>
            link: VueTSX.HTMLProps<HTMLLinkElement>
            main: VueTSX.HTMLProps<HTMLElement>
            map: VueTSX.HTMLProps<HTMLMapElement>
            mark: VueTSX.HTMLProps<HTMLElement>
            menu: VueTSX.HTMLProps<HTMLElement>
            menuitem: VueTSX.HTMLProps<HTMLElement>
            meta: VueTSX.HTMLProps<HTMLMetaElement>
            meter: VueTSX.HTMLProps<HTMLElement>
            nav: VueTSX.HTMLProps<HTMLElement>
            noindex: VueTSX.HTMLProps<HTMLElement>
            noscript: VueTSX.HTMLProps<HTMLElement>
            object: VueTSX.HTMLProps<HTMLObjectElement>
            ol: VueTSX.HTMLProps<HTMLOListElement>
            optgroup: VueTSX.HTMLProps<HTMLOptGroupElement>
            option: VueTSX.HTMLProps<HTMLOptionElement>
            output: VueTSX.HTMLProps<HTMLElement>
            p: VueTSX.HTMLProps<HTMLParagraphElement>
            param: VueTSX.HTMLProps<HTMLParamElement>
            picture: VueTSX.HTMLProps<HTMLElement>
            pre: VueTSX.HTMLProps<HTMLPreElement>
            progress: VueTSX.HTMLProps<HTMLProgressElement>
            q: VueTSX.HTMLProps<HTMLQuoteElement>
            rp: VueTSX.HTMLProps<HTMLElement>
            rt: VueTSX.HTMLProps<HTMLElement>
            ruby: VueTSX.HTMLProps<HTMLElement>
            s: VueTSX.HTMLProps<HTMLElement>
            samp: VueTSX.HTMLProps<HTMLElement>
            script: VueTSX.HTMLProps<HTMLElement>
            section: VueTSX.HTMLProps<HTMLElement>
            select: VueTSX.ChangeTargetHTMLProps<HTMLSelectElement>
            small: VueTSX.HTMLProps<HTMLElement>
            source: VueTSX.HTMLProps<HTMLSourceElement>
            span: VueTSX.HTMLProps<HTMLSpanElement>
            strong: VueTSX.HTMLProps<HTMLElement>
            style: VueTSX.HTMLProps<HTMLStyleElement>
            sub: VueTSX.HTMLProps<HTMLElement>
            summary: VueTSX.HTMLProps<HTMLElement>
            sup: VueTSX.HTMLProps<HTMLElement>
            table: VueTSX.HTMLProps<HTMLTableElement>
            tbody: VueTSX.HTMLProps<HTMLTableSectionElement>
            td: VueTSX.HTMLProps<HTMLTableDataCellElement>
            textarea: VueTSX.ChangeTargetHTMLProps<HTMLTextAreaElement>
            tfoot: VueTSX.HTMLProps<HTMLTableSectionElement>
            th: VueTSX.HTMLProps<HTMLTableHeaderCellElement>
            thead: VueTSX.HTMLProps<HTMLTableSectionElement>
            time: VueTSX.HTMLProps<HTMLElement>
            title: VueTSX.HTMLProps<HTMLTitleElement>
            tr: VueTSX.HTMLProps<HTMLTableRowElement>
            track: VueTSX.HTMLProps<HTMLTrackElement>
            u: VueTSX.HTMLProps<HTMLElement>
            ul: VueTSX.HTMLProps<HTMLUListElement>
